...
Z1_min = [please input value];
Z2_max = [please input value];

Z2_min = [please input value];
Z1_max = [please input value];
...
//12 SKUs - set 16 & 17
//range Carton = 1..3;   // Recycle
//range Plastic = 4..9; // Recycle
//range Glass = 10..12;  // Reused
//range Indirect = 1..9;
//range Direct = 10..12;
...
dexpr float Z1 = company_cost + sum(k in Recycled_pck) supplier_cost[k];

minimize lambda;

subject to {
	//Fuzzy Compromise constraint
	(Z1 - Z1_min) / (Z1_max - Z1_min) <= lambda;
	(Z2 - Z2_min) / (Z2_max - Z2_min) <= lambda; 
